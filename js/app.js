...
const EXPERTS_DATA = [
    ...
    { n: 'أ.د. سمير بن عيسى', s: 'النشر العلمي وتصنيف A/B', c: 'النشر العلمي والترقية', r: 4.9, p: 74, exp: '17 سنة', b: 'diamond', country: 'الجزائر', bio: 'متخصص في متطلبات الترقية الجامعية، تصنيف المجلات، وتوجيه الباحثين للنشر في Scopus وWeb of Science' },
    { n: 'د. مريم بوزيد', s: 'ASJP والمجلات الجزائرية', c: 'النشر العلمي والترقية', r: 4.8, p: 63, exp: '12 سنة', b: 'gold', country: 'الجزائر', bio: 'باحثة في فهرسة المجلات الجزائرية، تدقيق الملفات العلمية، وإعداد المقالات حسب شروط البوابات المحلية' },
    { n: 'أ.د. علي الربيعي', s: 'Clarivate وWeb of Science', c: 'النشر العلمي والترقية', r: 4.9, p: 69, exp: '18 سنة', b: 'diamond', country: 'العراق', bio: 'خبير في اختيار المجلات الدولية وتدقيق نطاق المجلة والرد على ملاحظات المحكمين' },
    { n: 'د. زينب الكاظمي', s: 'Scopus ومجلات الترقية', c: 'النشر العلمي والترقية', r: 4.8, p: 57, exp: '11 سنة', b: 'gold', country: 'العراق', bio: 'تعمل على تهيئة المقالات العلمية، تحسين الملخص، وتنسيق البحث حسب تعليمات الناشر' },
    { n: 'د. يزن الخطيب', s: 'منهجية البحث والنشر الدولي', c: 'النشر العلمي والترقية', r: 4.7, p: 52, exp: '13 سنة', b: 'gold', country: 'الأردن', bio: 'مستشار أكاديمي في تصميم الدراسات، اختيار المجلات، وخطط النشر للترقية والتوظيف' },
    { n: 'د. نور أبو زيد', s: 'التحليل الإحصائي SPSS/R', c: 'العلوم الطبيعية والبحتة', r: 4.8, p: 61, exp: '10 سنوات', b: 'gold', country: 'الأردن', bio: 'خبيرة في اختبار الفرضيات، بناء النماذج، وتفسير النتائج للرسائل والأوراق العلمية' },
    { n: 'م. كريم منصوري', s: 'MATLAB وSimulink', c: 'الهندسة والتقنية', r: 4.8, p: 46, exp: '9 سنوات', b: 'gold', country: 'المغرب', bio: 'ينفذ نماذج التحكم، معالجة الإشارات، ومحاكاة الأنظمة الهندسية لطلبة الدراسات العليا' },
    { n: 'د. أحمد مصطفى', s: 'ANSYS وCFD/FEM', c: 'الهندسة والتقنية', r: 4.7, p: 39, exp: '12 سنة', b: 'gold', country: 'مصر', bio: 'متخصص في المحاكاة الهندسية، تحليل الإجهادات، الموائع، والتقارير الفنية المصاحبة' },
    { n: 'م. رانيا مراد', s: 'Python والذكاء الاصطناعي', c: 'الهندسة والتقنية', r: 4.8, p: 58, exp: '8 سنوات', b: 'gold', country: 'تونس', bio: 'تطور مشاريع تعلم الآلة، تحليل البيانات، النماذج التنبؤية، ولوحات النتائج للباحثين' },
    { n: 'د. سارة العلي', s: 'COMSOL Multiphysics', c: 'الهندسة والتقنية', r: 4.7, p: 34, exp: '10 سنوات', b: 'gold', country: 'سوريا', bio: 'خبيرة في النمذجة متعددة الفيزياء، إعداد السيناريوهات، وتوثيق نتائج المحاكاة علمياً' }
];
...
const CERTIFICATES_DATA = [
    { t: 'شهادة نشر دولي', d: 'شهادة مرتبطة بخدمة النشر العلمي، ويمكن رفع صورتها أو ملفها لاحقاً.', icon: 'fa-newspaper', tag: 'نشر' },
...
function initAuth() {
    ...
    document.getElementById('loginForm').addEventListener('submit', function(e) {
        ...
        const users = JSON.parse(localStorage.getItem('ahu_users') || '[]');
        const found = users.find(u => String(u.email).toLowerCase() === em);
        if (!found) return showAuthMsg('هذا البريد غير مسجل. يرجى إنشاء حساب أولاً.', 'error');
        if (found.password !== pw) return showAuthMsg('كلمة المرور غير صحيحة.', 'error');
        const user = { ...found, password: undefined, isLoggedIn: true };
    ...
    document.getElementById('registerForm').addEventListener('submit', function(e) {
        ...
        const users = JSON.parse(localStorage.getItem('ahu_users') || '[]');
        if (users.some(u => String(u.email).toLowerCase() === em)) return showAuthMsg('هذا البريد مسجل مسبقاً. يمكنك تسجيل الدخول مباشرة.', 'error');
        const storedUser = { firstName: fn, lastName: ln, fullName: fn + ' ' + ln, name: fn + ' ' + ln, email: em, academicLevel: lv, specialization: sp, password: pw, createdAt: new Date().toISOString() };
        users.push(storedUser);
        localStorage.setItem('ahu_users', JSON.stringify(users));
        const user = { ...storedUser, password: undefined, isLoggedIn: true };
        localStorage.setItem('ahu', JSON.stringify(user));
        showAuthMsg('✅ تم إنشاء الحساب بنجاح. تم ربط الدخول بحساب مسجل فقط.', 'success');
        setTimeout(() => location.href = 'dashboard.html', 600);
    });
}
...
function checkUser() {
    ...
    ST.orders = ST.user && ST.user.email ? JSON.parse(localStorage.getItem(getUserStorageKey('ahu_orders')) || '[]') : [];
    ST.notifs = ST.user && ST.user.email ? JSON.parse(localStorage.getItem(getUserStorageKey('ahu_notifs')) || '[]') : [];
}
...
function addNotification(type, msg, icon, color) {
    ...
    localStorage.setItem(getUserStorageKey('ahu_notifs'), JSON.stringify(ST.notifs));
    updateNotifUI();
    updateAllBadges();
}
...
function renderCertificatesGrid() {
    ...
    grid.innerHTML = CERTIFICATES_DATA.map((c, i) => `
        <article class="cert-card">
            ...
            <h4>${c.t}</h4>
            <p>${c.d}</p>
            <button type="button" class="cert-view-btn" aria-label="عرض الشهادة">عرض الشهادة</button>
        </article>
    `).join('');
}
...
function initAllForms() {
    const forms = [
        ...
        { id: 'programmingForm', name: 'خدمة البرمجة والذكاء الاصطناعي' },
        { id: 'simulationForm', name: 'خدمة المحاكاة والنمذجة' },
        { id: 'promotionForm', name: 'خدمة الترقية والتوظيف الأكاديمي' }
    ];
...
function loadExpertsGrid() {
    ...
    grid.innerHTML = experts.map(e => {
        const badgeText = e.country ? `🌍 ${e.country}` : e.c;
        return `
        <div class="exp-card">
            <span class="exp-badge exp-badge-${e.b}">${badgeText}</span>
            <div class="exp-avatar"><i class="fa-solid fa-user-tie"></i></div>
            <h4>${e.n}</h4>
            <p class="exp-spec">${e.s}</p>
            ${e.country ? `<p class="exp-country"><i class="fa-solid fa-location-dot"></i> ${e.country}</p>` : ''}
...
function initFinalPolish(){
    ...
    document.addEventListener('click', function(e){
        ...
        if(btn){
            e.preventDefault();
            btn.classList.add('cert-pulse');
            setTimeout(()=>btn.classList.remove('cert-pulse'),420);
        }
    });
    const pc=document.getElementById('pageContainer');
    if(pc) pc.style.scrollBehavior='smooth';
}
